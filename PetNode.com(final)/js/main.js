...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ADMIN_TOKEN_KEY = 'petnode_admin_token';
    ...
    // ── Toggle switch: redirect to dashboard or prompt login ──
    if (toggleInput) {
        toggleInput.addEventListener('change', function() {
            if (toggleInput.checked) {
                if (isAdminLoggedIn()) {
                    ...
                    // Reset toggle, open login modal to authenticate first
                    toggleInput.checked = false;
                    openModal();
                    // Switch directly to admin login view
    ...
    // Admin login → save token → redirect to dashboard
    if (adminBtn) {
