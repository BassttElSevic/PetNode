...
import urllib3  # HTTP 底层库（用于禁用 InsecureRequestWarning）
...
# 也支持 https:// 前缀（通过 nginx 反向代理访问 HTTPS）
...
class HttpExporter(BaseExporter):
    ...
    def __init__(
        self,
        api_url: str = _DEFAULT_API_URL,  # Flask 服务器地址（支持 http:// 和 https://）
        ...
        verify_ssl: bool = True,  # 是否验证 SSL 证书（自签名证书需设为 False）
    ) -> None:
        ...
        # 保存 SSL 证书验证选项
        # verify_ssl=False 时跳过证书验证（自签名证书不被系统信任，需要此选项）
        self._verify_ssl = verify_ssl

        # 当 verify_ssl=False 时，禁用 urllib3 的 InsecureRequestWarning 告警
        # 避免每次请求都打印"InsecureRequestWarning: Unverified HTTPS request"
        if not verify_ssl:
            urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
            logger.warning("SSL 证书验证已禁用（verify_ssl=False），仅用于自签名证书测试环境")
        ...
        # 记录初始化日志
        logger.info("HttpExporter 已就绪: url=%s, cache=%s, verify_ssl=%s", self._url, self._cache_dir, self._verify_ssl)
    ...
    def _sign_and_post(self, record: dict) -> requests.Response:
        ...
        # 确保发送的字节流与签名时完全一致
        # verify=self._verify_ssl: 控制是否验证 SSL 证书（自签名证书需设为 False）
