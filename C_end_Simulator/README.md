...
当前统一约定：Engine 上报的 record 只保留设备遥测字段，不携带 `user_id`；`user_id` 只保留在 Flask 的身份、绑定与家庭组域，以及 TUI 的本地会话语义中。
...
└── ui_tui/                             <-- 【终端交互界面 PetNodeOS (Textual，打包进 Docker)】
    ...
    │   └── user_store.py               <-- ⏳ 占位文件（后续承接 TUI 本地会话管理）
    │
    └── screens/                        <-- 【TUI 前端界面层】
        ├── __init__.py
        ├── login_screen.py             <-- ⏳ 占位文件（登录屏待实现）
...
flowchart TD
    ...
    subgraph DockerEnv["🐳 Docker 环境 (打包给老师的部分)"]
        ...
        subgraph TUIContainer["ui_tui/ 容器 (PetNodeOS 交互式终端·打包给老师)"]
            ...
            tui_backend["ui_tui/backend/<br/>后端接口层<br/>DataAPI·CommandAPI·UserStore(占位)"]
            login_scr["ui_tui/screens/login_screen.py<br/>占位文件<br/>登录屏待实现"]
...
#### 3. UserStore（用户管理接口，占位说明）

**位置**：`ui_tui/backend/user_store.py`

当前该文件在代码中仍为空，以下内容仅为设计目标。
...
#### 登录屏（LoginScreen，占位说明）

当前 `ui_tui/screens/login_screen.py` 也仍为空，以下内容仅为设计目标。
...
- `user_store.py`：当前为空文件，后续可承接 TUI 本地会话管理

**前端界面层（screens/）**：
- `login_screen.py`：当前为空文件，登录屏待实现
...
查询接口支持的常用参数：`source=mongo|mysql`、`kind=records|anomalies|profile`、`user_id`、`device_id`、`start_time`、`end_time`、`limit`、`offset`。其中 `user_id` / `device_id` 只用于 Flask 侧的身份查询、绑定查询和档案查询，不属于 Engine 上传 record 的字段；`/api/data` 的请求体仍只包含设备遥测数据。Mongo 默认返回实时记录；MySQL 默认返回异常记录，`kind=profile` 时返回固定档案信息。
...
| `test_step5_tui_backend.py` | Step 5 | DataAPI、CommandAPI、UserStore 规划中的单元测试 | 否 |
...
##### UserStore（用户管理接口，占位说明）

**位置**：`ui_tui/backend/user_store.py`

当前该文件在代码中仍为空，以下内容是预期设计，不代表已实现。
...
#### 7.3.3 UserStore —— 用户管理接口（规划中）

> 当前 `ui_tui/backend/user_store.py` 仍为空文件，以下内容仅表示设计目标。
