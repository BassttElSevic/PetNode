...
当前统一口径：Engine 上报的遥测 record 不包含 `user_id`；`user_id` 仅用于 Flask 侧身份、绑定和权限域，MySQL 存储层只在历史兼容或旧数据回填时处理缺省值。
...
| MySQL 内部 | 方法 | `MySQLStorage.save(record)` | 写入异常记录（anomaly）；Engine record 不含 `user_id` |
| MySQL 内部 | 方法 | `MySQLStorage.query_anomalies(...)` | 按条件查询异常列表 |
| MySQL 内部 | 方法 | `MySQLStorage.query_profile(...)` | 查询 user/device/trait/event 档案 |
| MySQL 内部 | 方法 | `MySQLStorage._resolve_user_id_from_record(record, now)` | 历史兼容：仅对旧数据或补录记录做 `user_id` 回填；Engine 新上报 record 不携带 `user_id` |
| MySQL 内部 | 方法 | `MySQLStorage._ensure_device(device_sn, now, user_id)` | 设备 upsert 并维护归属字段；`user_id` 仅用于 Flask 绑定域，不属于 Engine 载荷 |
...
| `MySQLStorage._resolve_user_id_from_record` | `record.user_id?` | 解析后的 `user_id` | 历史兼容入口：仅用于旧数据回填，Engine 新上报 record 不需要 `user_id` |
